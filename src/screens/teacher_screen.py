...
def teacher_tab_take_attendanc():
    ...
    if st.session_state.attendance_images:
        ...
        with c1:
            if st.button("Clear all photo",width='stretch',type='tertiary',icon=':material/delete:' ,disabled= not has_photo):
                st.session_state.attendance_images=[]
                st.rerun()
            ...
            if st.button("Run Face analysis",width='stretch',type='secondary',icon=':material/analytics:',disabled=not has_photo):
                with st.spinner("Deep scanning classroom photos..."):
                    ...
                    if not enrolled_students:
                        ...
                        current_timestamp=datetime.now().strftime("%Y_%m_%dT%H:%M:%S")
...
def teacher_tab_attendance_records():
    ...
    for r in records:
        ...
        data.append({
            ...
            "Time": datetime.fromisoformat(ts).strftime("%Y_%m_%d %I:%M %p") if ts else "N'A",
